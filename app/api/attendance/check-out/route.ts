...
export async function POST(request: Request) {
  try {
    ...
    if (demo) {
      ...
      return NextResponse.json({ event: { id: eventId, record_code: `ATT-${eventId.slice(0, 8).toUpperCase()}`, event_type: "CHECK_OUT", server_timestamp: new Date().toISOString() }, session: { id: crypto.randomUUID(), status: "COMPLETE", check_out_time: new Date().toISOString(), duration_seconds: 33060 }, demo: true });
    }
    const data = await checkOut({ profile, photo, clientCaptureTime: typeof clientCaptureTime === "string" ? clientCaptureTime : null });
