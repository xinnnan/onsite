...
const styles = StyleSheet.create({
  page: { padding: 34, fontSize: 8, color: "#243b35", fontFamily: "Helvetica" },
  ...
  projectMap: { width: "100%", height: 118, objectFit: "cover", marginTop: 10, border: "1 solid #dce5e0" },
  ...
  photoPage: { padding: 34, fontSize: 8, color: "#243b35", fontFamily: "Helvetica" }, photoHeading: { fontSize: 13, fontWeight: 700, color: "#0f4c3a" },
...
export async function POST(request: Request) {
  try {
    ...
    const report = demo ? { sessions: [], selectedProject: null, summary: { total_personnel: 0,total_work_sessions:0,total_work_hours:0,total_work_days:0,incomplete_sessions:0 }, personnel: [] } : await buildReportData({ projectId: body.project_id, workerId: body.worker_id, start: body.start, end: body.end });
    ...
    const doc = h(Document, null,
      h(Page, { size: "A4", style: styles.page },
        h(Text, { style: styles.brand }, "DROPLETAI SERVICES"),
        ...
        h(View, { style: styles.metadata },
          ...
          h(View, { style: styles.metaBlock }, h(Text,{style:styles.label},"Site"),h(Text,{style:styles.value},project.siteName),h(Text,{style:styles.label},"Address"),h(Text,{style:styles.value},project.address),h(Text,{style:styles.label},"Reporting period"),h(Text,{style:styles.value},`${body.start || "All"} — ${body.end || "All"}`))),
        projectMapUrl ? h(Image, { style: styles.projectMap, src: projectMapUrl }) : null,
