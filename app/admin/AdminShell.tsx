...
import {
  ...
  Languages, LayoutDashboard, LoaderCircle, MapPin, Menu, MoreHorizontal, Plus,
  Search, ShieldCheck, SlidersHorizontal, Upload, Users, UserRoundCheck, X,
} from "lucide-react";
...
const demoProjects = [
  { id:"adidas-indy",project_code:"ADI-AMR-26",project_name:"adidas Indy AMR",customer_name:"adidas",site_name:"Indy Manufacturing Facility",address_line_1:"8677 Impact Court",city:"Indianapolis",state:"IN",postal_code:"46219",country:"United States",timezone:"America/Indiana/Indianapolis",status:"ACTIVE" },
  { id:"walmart-atlanta",project_code:"WMT-ATL-04",project_name:"Walmart Atlanta",customer_name:"Walmart",site_name:"ATL Distribution Center",address_line_1:"1200 Logistics Pkwy",city:"Norcross",state:"GA",postal_code:"30071",country:"United States",timezone:"America/New_York",status:"ACTIVE" },
];
const demoSessions = [
  { id:"demo-session-1",check_in_time:"2026-08-17T12:03:00Z",check_out_time:"2026-08-17T21:14:00Z",duration_seconds:33060,status:"COMPLETE",worker:demoPeople[0],project:demoProjects[0],check_in_event:{record_code:"ATT-000123-I"},check_out_event:{record_code:"ATT-000123-O"} },
...
function formatDate(value?: string, locale = "en-US") { return value ? new Intl.DateTimeFormat(locale,{month:"short",day:"2-digit",year:"numeric"}).format(new Date(value)) : "—"; }
function formatTime(value?: string, locale = "en-US") { return value ? new Intl.DateTimeFormat(locale,{hour:"2-digit",minute:"2-digit"}).format(new Date(value)) : "—"; }
...
function PageState({ loading, error, empty, retry, children, t }: { loading:boolean;error:string;empty:boolean;retry:()=>void;children:ReactNode;t:typeof text.zh|typeof text.en }) {
...
export default function AdminShell({ view }: { view: AdminView }) {
  ...
  return <div className="admin-shell">
    ...
    {menuOpen&&<button className="menu-scrim" onClick={()=>setMenuOpen(false)}/>}<main className="admin-main"><header className="admin-topbar"><button className="menu-button" onClick={()=>setMenuOpen(true)}><Menu size={20}/><span>{t.menu}</span></button><div className="admin-search"><Search size={18}/><input placeholder={t.search}/></div><div className="admin-actions"><button onClick={toggleLanguage}><Languages size={16}/><span>{t.language}</span></button><button className="notification"><Bell size={18}/><i/></button></div></header>
      <div className="admin-page"><div className="page-title-row"><div><p>{t.today} · {formatDate(new Date().toISOString(),locale==="zh"?"zh-CN":"en-US")}</p><h1>{t[activeNav as keyof typeof t]}</h1></div>{(view==="dashboard"||view==="users")&&<button className="admin-primary" onClick={()=>setModal({type:"worker"})}><Plus size={17}/>{t.addWorker}</button>}{view==="projects"&&<button className="admin-primary" onClick={()=>setModal({type:"project"})}><Plus size={17}/>{t.addProject}</button>}</div>
...
function ProjectDetail({data,loading,error,load,t,flash}:{data:Row;loading:boolean;error:string;load:()=>void;t:T;flash:(s:string)=>void}) {
  ...
  async function save(event:FormEvent<HTMLFormElement>){
    ...
    for(const key of ["address_line_2","site_name","state","postal_code","start_date","end_date"]){if(body[key]==="")body[key]=null}
...
function ReportsView({data,loading,error,load,t,flash}:{data:Row;loading:boolean;error:string;load:()=>void;t:T;flash:(s:string)=>void}) { const projects=data.demo?demoProjects:(data.projects||[]);const users=data.demo?demoPeople:(data.users||[]);async function download(formElement:HTMLFormElement,type:"pdf"|"xlsx"|"csv"){const form=new FormData(formElement);const body={project_id:form.get("project_id"),worker_id:form.get("worker_id")||undefined,start:form.get("start"),end:form.get("end"),include_photos:form.get("include_photos")==="on"};const response=await fetch(`/api/admin/reports/${type}`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(body)});if(!response.ok)return;const blob=await response.blob();const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=`onsite-report.${type}`;link.click();URL.revokeObjectURL(url);flash(t.downloaded);}return <PageState loading={loading} error={error} empty={!projects.length} retry={load} t={t}><div className="report-layout"><form className="admin-card report-builder" onSubmit={(event)=>{event.preventDefault();download(event.currentTarget,"pdf")}}><div className="report-icon"><FileText size={24}/></div><h2>{t.reportTitle}</h2><label>{t.project}<select name="project_id">{projects.map((p:Row)=><option key={p.id} value={p.id}>{p.project_name}</option>)}</select></label><label>{t.worker}<select name="worker_id"><option value="">—</option>{users.map((user:Row)=><option key={user.id} value={user.id}>{user.display_name}</option>)}</select></label><label>{t.period}<div className="date-pair"><input name="start" type="date"/><span>—</span><input name="end" type="date"/></div></label><label className="check-label"><input name="include_photos" type="checkbox"/><span>✓</span>{t.include}</label><div className="report-actions"><button className="admin-primary" type="submit"><FileText size={16}/>{t.pdf}</button><button className="secondary-button" type="button" onClick={(event)=>event.currentTarget.form&&download(event.currentTarget.form,"xlsx")}><ArrowDownToLine size={16}/>{t.excel}</button><button className="secondary-button" type="button" onClick={(event)=>event.currentTarget.form&&download(event.currentTarget.form,"csv")}><ArrowDownToLine size={16}/>{t.csv}</button></div></form><article className="report-preview"><div className="report-paper"><header><div><strong>DROPLETAI</strong><span>SERVICES</span></div><p>SITE ATTENDANCE REPORT</p></header><h3>{t.project}</h3><p>{projects[0]?.project_name}</p></div></article></div></PageState> }
...
function AdminModal({modal,t,close,saved}:{modal:NonNullable<ModalState>;t:T;close:()=>void;saved:()=>void}) {
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    if(modal.type==="project"){url="/api/admin/projects";body=Object.fromEntries(form.entries());for(const key of ["address_line_2","site_name","state","postal_code","start_date","end_date"]){if(body[key]==="")body[key]=null}}
